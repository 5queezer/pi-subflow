...
- `subflow` tool: accepts `agent` + `task`, `chain`, or `tasks` and dispatches to single, chain, parallel, or DAG execution. In interactive Pi sessions it shows a progress widget with mode/status, completion/failure/skipped counts, elapsed time, timeout, and per-task `✓`/`✗`/`⏳` rows, then clears the widget at completion. Final tool output is a compact summary card with task-level output/error lines, collapsed long outputs, a `final:` line when available, and a simple ASCII DAG tree for dependency runs.
