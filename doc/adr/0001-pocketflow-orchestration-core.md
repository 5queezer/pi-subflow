...
- As of the DAG node integration slice, DAG execution is PocketFlow node-backed internally: validation, max-turns preflight, stage execution, verifier repair, and result aggregation run through explicit internal PocketFlow nodes. Chain and parallel modes are also now PocketFlow node-backed. In addition, the Pi extension adapter now treats the overall `executeSubflow` orchestration as a coarse-grained PocketFlow flow that captures extension-level phases (normalization, policy validation, agent discovery, runner preparation, flow execution, history persistence, result formatting) so Pi tool output includes those as `pocketflow_node` trace phases.
